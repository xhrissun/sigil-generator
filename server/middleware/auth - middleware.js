...
const canUserGenerate = (user) => {
  ...
  // Check daily limit
  if (user.dailyGenerations >= user.dailyLimit) {
    ...
    return {
      ...
      message: `Daily generation limit reached (${user.dailyLimit}). Upgrade to premium for unlimited generations.`,
      resetTime: tomorrow.toISOString()
    }
  }

  // Check monthly limit
  if (user.monthlyGenerations >= user.monthlyLimit) {
    const nextMonth = new Date(currentYear, currentMonth + 1, 1)
    
    return {
      allowed: false,
      reason: 'monthly_limit_reached',
      message: `Monthly generation limit reached (${user.monthlyLimit}). Upgrade to premium for unlimited generations.`,
  ...
  try {
    ...
    if (!canGenerate.allowed) {
      return res.status(429).json({
        ...
        upgradeUrl: '/upgrade',
        currentUsage: {
          daily: req.user.dailyGenerations,
          monthly: req.user.monthlyGenerations,
...
export const recordGeneration = async (req, res, next) => {
  try {
    ...
    // Update the user object in request for response
    req.user = user
