...
const getUserUsageStats = async (user) => {
  const now = new Date()
  ...
  let dailyGenerations = user.usageStats.dailyGenerations || 0
  let monthlyGenerations = user.usageStats.monthlyGenerations || 0
  ...
  if (!lastGenDate || lastGenToday < today) {
    ...
    user.usageStats.remainingDaily = user.usageStats.dailyLimit
    resetNeeded = true
  }
  ...
  if (!lastGenDate || lastGenYear < currentYear || (lastGenYear === currentYear && lastGenMonth < currentMonth)) {
    ...
    user.usageStats.remainingMonthly = user.usageStats.monthlyLimit
  ...
  const dailyLimit = user.isPremium ? Number.MAX_SAFE_INTEGER : (user.usageStats.dailyLimit || 5)
  const monthlyLimit = user.isPremium ? Number.MAX_SAFE_INTEGER : (user.usageStats.monthlyLimit || 50)
  ...
  return {
    ...
    lastGenerationDate: user.usageStats.lastGenerationDate
  }
}
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!process.env.JWT_SECRET || !process.env.JWT_REFRESH_SECRET) {
      throw new Error('JWT_SECRET or JWT_REFRESH_SECRET is undefined');
    }
    const token = generateToken(user._id);
    const refreshTokenValue = generateRefreshToken(user._id);
    console.log('Updating user login stats...');
    user.loginCount += 1;
    user.lastLoginDate = new Date();
    await user.save();
    ...
    // Convert user to JSON and exclude password
...
router.post('/logout', authenticate, async (req, res) => {
  try {
    // Optionally invalidate tokens server-side (e.g., add to a blacklist)
    res.json({
      success: true,
...
router.get('/can-generate', authenticate, async (req, res) => {
  try {
    ...
    // Check limits
    const dailyLimit = user.usageStats.dailyLimit || 5
    const monthlyLimit = user.usageStats.monthlyLimit || 50
